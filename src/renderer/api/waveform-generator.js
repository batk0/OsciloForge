...
export function generateTriangleWave(amplitude, cycles, points) {
    ...
    for (let i = 0; i < points; i++) {
        const phaseInPeriod = i % periodPoints;
        if (phaseInPeriod < quarterPeriod) { // First quarter: 0 to amplitude
            data[i] = amplitude * (phaseInPeriod / quarterPeriod);
        } else if (phaseInPeriod < 2 * quarterPeriod) { // Second quarter: amplitude to 0
            data[i] = amplitude * (1 - (phaseInPeriod - quarterPeriod) / quarterPeriod);
        } else if (phaseInPeriod < 3 * quarterPeriod) { // Third quarter: 0 to -amplitude
            data[i] = -amplitude * ((phaseInPeriod - 2 * quarterPeriod) / quarterPeriod);
        } else { // Fourth quarter: -amplitude to 0
            data[i] = -amplitude * (1 - (phaseInPeriod - 3 * quarterPeriod) / quarterPeriod);
        }
    }
    return data;
}
