...
import { state, updateState, WAVEFORM_POINTS, TOP_PADDING, RIGHT_PADDING, BOTTOM_PADDING, LEFT_PADDING } from './state.js';
...
export class UIManager {
    ...
    setupShiftListeners() {
        if (!this.shiftLeftBtn || !this.shiftRightBtn || !this.shiftUpBtn || !this.shiftDownBtn) {
            console.warn('Shift buttons not found');
            return;
        }
    ...
    setupZoomListeners() {
        if (!this.hZoomSlider || !this.vZoomSlider) {
            console.warn('Zoom sliders not found');
            return;
        }
    ...
    setupDrawStyleListeners() {
        if (!this.drawStyleLine || !this.drawStyleDots) {
            console.warn('Draw style elements not found');
            return;
        }
    ...
    setupEditModeListeners() {
        ...
        if (editModeRadios.length === 0) {
            console.warn('Edit mode radio buttons not found');
            return;
        }
    ...
    setupFileOperationListeners() {
        if (!this.openBtn || !this.saveBtn || !this.resetBtn) {
            console.warn('File operation buttons not found');
            return;
        }
    ...
    setupWaveformGenerationListeners() {
        if (!this.generateWaveformBtn || !this.waveformTypeSelect || !this.amplitudeInput || !this.cyclesInput || !this.dutyCycleInput) {
            console.warn('Waveform generation elements not found');
            return;
        }
    ...
    setupDeviceDownloadListener() {
        if (!this.downloadDeviceBtn) {
            console.warn('Download device button not found');
            return;
        }
